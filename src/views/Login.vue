...
const emailLogin = async () => {
  ...
  try {
    const response = await get(`/login?username=${encodeURIComponent(emailForm.email)}&password=${encodeURIComponent(emailForm.password)}`);
    if (response.status === 1 || response.code === 200) {
      handleLoginSuccess(response);
    } else {
      const errMsg = response.msg || response.message || '';
      $message.error(errMsg || t('deng-lu-shi-bai'));
      console.error('[Login] email login failed:', response);
    }
  } catch (error) {
    console.error('[Login] email login error:', error.response?.data || error);
    $message.error(error.response?.data?.data || error.response?.data?.msg || t('deng-lu-shi-bai'));
  } finally {
...
const phoneLogin = async (selectedUserId = null) => {
  ...
  try {
    ...
    if (response.status === 1 || response.code === 200) {
      ...
      const errMsg = response.msg || response.message || response.data?.msg || '';
      $message.error(errMsg || t('deng-lu-shi-bai'));
      console.error('[Login] phone login failed:', response);
    }
  } catch (error) {
    ...
    console.error('[Login] phone login error:', error.response?.data || error);
  } finally {
...
const handleLoginSuccess = (response) => {
    MoeAuth.setData({ UserInfo: response.data || response });
