    ...
    <div class="settings-page">
        ...
        <div v-if="isSelectionOpen" class="modal" @click.self="closeSelection">
            <div class="modal-content">
                ...
                <div v-if="selectionType === 'spectrumScale'" class="scale-slider-container">
                    <div class="scale-slider-label">频谱幅度: {{ spectrumScale.toFixed(1) }}x</div>
                    <div class="scale-slider-wrapper">
                        <input type="range" min="0.2" max="3" step="0.1" v-model.number="spectrumScale"
                            class="scale-slider" @input="previewSpectrumScale" @change="saveSpectrumScale" />
                        <div class="scale-marks">
                            <span>0.2</span>
                            <span>1.0</span>
                            <span>2.0</span>
...
const saveSettings = () => {
    ...
    if (isElectron()) {
        ...
        // 实时通知频谱窗口
        for (const key of ['spectrumLocked', 'spectrumScale']) {
...
const previewSpectrumScale = () => {
    ...
    window.electron.ipcRenderer.send('spectrum-setting-update', {
        key: 'spectrumScale',
        value: spectrumScale.value.toFixed(1)
    });
};

const saveSpectrumScale = () => {
    const value = spectrumScale.value.toFixed(1);
    selectedSettings.value.spectrumScale = {
        displayText: `${value}x`,
        value
    };
    saveSettings();
